...
export default {
  ...
  mounted () {
    this.setTitle('Editar vacante')
    this.fetchSingleVacante(this.id).then(res => {
      this.vacante = res
    })
    this.fetchEstados()
  ...
  methods: {
    ...mapActions({
      ...
      fetchSingleVacante: 'empleo/fetchSingleVacante',
      fetchEstados: 'resources/fetchEstados',
      fetchPrestaciones: 'resources/fetchPrestaciones',
      updateVacante: 'empleo/updateVacante'
    }),
    onSaveVacante () {
      this.updateVacante(this.vacante).then(() => {
